...
hgnc_annot <- function(df) {
  ...
  library(org.Hs.eg.db)
    ...
    genes = c(df1$Gene_ID)
      df2 <- select(org.Hs.eg.db, keys = genes, keytype = "ENSEMBL", columns = c("ENSEMBL", "SYMBOL"))
...
```{r read_geneset -- reads a text file of gene sets}

read_geneset = function(path_to_gset){
  library(GSA)
...
all(rownames(metadata) == colnames(expr_data)) 
  ...
  metadata$Dx <- factor(metadata$Dx, levels = c("Control", "SCZ"))
    ...
    metadata_design <- model.matrix(~ metadata$Ethnicity + metadata$Dx + metadata$Ethnicity*metadata$Dx + scale(metadata$RIN) + scale(metadata$IntronicRate) + scale(metadata$IntragenicRate) + scale(metadata$IntergenicRate) + scale(metadata$rRNARate) + metadata$Institution*(metadata$ageOfDeath + metadata$cellFrac_ilr_1 + metadata$cellFrac_ilr_2 + metadata$cellFrac_ilr_3), data = metadata)
...
library(ggh4x)
  ...
  scores = scores %>% tidyr::pivot_longer(cols = 2:ncol(df_scores), names_to = "SampleID", 
...
race_enrichment_scores <- plot_enrichment_race(GSVA_results$GSVA_race, 
                                           ...
                                           METADATA, 
                                           pathways = c("positive regulation of MAP kinase activity (GO:0043406)",
                                                        "natural killer cell activation (GO:0030101)",
                                                        "negative regulation of oxidative stress-induced cell death (GO:1903202)",
                                                        "positive regulation of neutrophil migration (GO:1902624)"))

...
volcano_race <- plot_volcano_race(GSVA_results$GSVA_race, xlim = c(-5, 5), ylim = c(0, 18))
print(volcano_race)
...
ggsave('volcano_inter.png', width = 7.5, height = 7, path = "~/Desktop/School/UofT MSc Medical Science/Research/Thesis Research/Plots/Interaction/Mega-Analysis")
...
sig_race <- df_race %>% dplyr::filter(abs(logFC) > 2 & adj.P.Val < 0.05)
sig_inter<- df %>% dplyr::filter(abs(logFC) > 0.25 & adj.P.Val < 0.05)
